...
namespace Invector.vCharacterController.AI
{
    ...
    public class vSimpleMeleeAI_Controller : vSimpleMeleeAI_Animator, vIMeleeFighter
    {
        ...
        // If hit by a GameObject tagged as `bulletTag`, reduce AI health by `bulletDamage`.
        void OnTriggerEnter(Collider other)
        {
            if (other == null) return;
            if (other.CompareTag(bulletTag))
            {
                currentHealth = Mathf.Max(0f, currentHealth - bulletDamage);

                if (!passiveToDamage)
                {
                    SetCurrentTarget(other.transform);
                    currentState = AIStates.Chase;
                    SetAgressive(true);
                }
        ...
        void OnCollisionEnter(Collision collision)
        {
            ...
            if (collision.gameObject.CompareTag(bulletTag))
            {
                ...
                if (!passiveToDamage)
                {
                    SetCurrentTarget(collision.transform);
                    currentState = AIStates.Chase;
                    SetAgressive(true);
                }
